...
export default {
    ...
    methods:{
      beforePrint(selectedRow,gridName,vtitle){
        ...
        outFilePageSizeChange(val){
          this.outFilePageSize = val;
        ...
        putInstoreFile(){
          ...
          axios.post("/dc/putInstoreFile",JSON.stringify(p))
        ...
        putInOrder(){
          ...
          axios.post("/dc/putInStorageBorrow",JSON.stringify(m))
            .then(function(response) {
              _self.loadGridData();
              _self.loadFileGridData();
              _self.loadOutFileGridData();
        ...
        loadGridData() {
          ...
          if (key0 && key0 != "") {
            key0 = " (coding like '%" + key0 + "%' or C_DRAFTER like '%" + key0 + "%') and STATUS='待入库' ";
          }else{
              key0=" STATUS='待入库' "
          }
