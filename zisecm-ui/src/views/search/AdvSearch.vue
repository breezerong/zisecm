...
export default {
  ...
  methods: {
    refreshData() {
      ...
      axios.get('/workflow/getMyDoneTask',JSON.stringify(m))
    ...
    loadPageInfo() {
      ...
      var m = new Map();
      m.set("condition", _self.inputkey);
      axios.get("/workflow/getMyDoneCount",JSON.stringify(m))
        .then(function(response) {
    ...
    showitem(indata) {
      let _self = this;
      axios.get("/workflow/getWorkflowTask",JSON.stringify(indata.workflowId))
        .then(function(response) {
