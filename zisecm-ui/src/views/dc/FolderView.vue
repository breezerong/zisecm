<template>
  <div>
    <el-dialog title="替换文件" :visible.sync="udialogVisible">
...
export default {
  ...
  methods: {
    ...
    showUpdateFile(inData){
      this.updateId = inData.ID;
      this.udialogVisible = true;
    },
    updateNewFile() {
      let _self = this;
        let formdata = new FormData();
        formdata.append("id",_self.updateId);
        if(_self.uploadFile!="")
        {
          console.log(_self.updateFile);
    ...
    loadFolder(){
      let _self = this;
      _self.loading = true;
      axios.post('/folder/getFolderByConfige',_self.paraName)
        .then(function(response) {
    ...
    loadGridData(indata) 
    {
      ...
      axios.post("/dc/getDocuments",SON.stringify(m))
      .then(function(response) {
