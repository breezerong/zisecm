            ...
            <template slot-scope="scope">
              <router-link
                :to="{
                  ...
                  query: { parentid: scope.row.id, name: scope.row.name },
                }"
...
export default {
  ...
  created() {
    let _self = this;
    let systemPermission = Number(this.currentUser().systemPermission);
    if (systemPermission < 9) {
      _self.$router.push({ path: "/NoPermission" });
    }
    _self.loading = true;
    axios
      .get("/admin/getGridView")
    ...
    axios
      .post("/query/getquery", JSON.stringify(queryObj))
      .then(function (resp) {
  ...
  methods: {
    ...
    saveitem(indata) {
      let _self = this;
      axios
        .post("/admin/updateGridView", JSON.stringify(indata))
        .then(function (response) {
          _self.$message("保存成功!");
        })
        .catch(function (error) {
    ...
    delitem(indata) {
      let _self = this;
      axios
        .post("/admin/deleteGridView", JSON.stringify(indata))
        .then(function (response) {
          _self.$message("删除成功!");
          _self.refreshData();
        })
    ...
    additem(indata) {
      let _self = this;
      axios
        .post("/admin/newGridView", JSON.stringify(indata))
        .then(function (response) {
    ...
    copyitem(indata) {
      let _self = this;
      axios
        .post("/admin/copyGridView", JSON.stringify(indata))
        .then(function (response) {
          _self.$message("复制成功!");
          _self.dialogVisible = false;
          _self.refreshData();
