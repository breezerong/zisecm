<template>
  <el-form label-width="120px" v-loading="loading" @submit.native.prevent>
    <el-row>
      <el-col :span="8">
        <el-form-item :label="$t('application.ImportTemplate')">
...
export default {
  ...
  methods: {
    loadTemplate(){
      let _self = this;
      _self.loading = true;
      axios.get("/IEDimport/getImportTemplates").then(function(response) {
    ...
    batchImport() {
      ...
      // _self.loading = true;
      axios
        .post("/IEDimport/batchImport", formdata, {headers:{
        ...
        .then(function(response) {
          ...
          // _self.loading = false;
          _self.$message(_self.$t('application.importSuccess'));
          _self.cleanFiles();
