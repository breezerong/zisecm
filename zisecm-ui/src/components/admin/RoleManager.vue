...
export default {
  ...
  methods: {
    ...
    removeUser(indata) {
      ...
      _self
        .axios({
          ...
          method: "post",
          data: JSON.stringify(m),
          url: "/zisecm/admin/removeUserRole"
        })
        .then(function(response) {
          _self.$message("移出成功!");
          _self.refreshUserData();
    ...
    loadUserPageInfo(pager) {
      ...
      _self
        .axios({
          ...
          method: "post",
          data: JSON.stringify(m),
          url: "/zisecm/admin/getRoleUserCount"
        })
        .then(function(response) {
        ...
        for (i in tab) {
          if (_self.inputkey != "") {
            if (
              ...
              tab[i].description.indexOf(_self.inputkey) >= 0
            ) {
    ...
    searchUser() {
      let _self = this;
      let tab = _self.userListFull;
      _self.userList = [];
      var i;
      if (_self.userInputkey != "") {
        for (i in tab) {
          if (_self.userInputkey != "") {
            if (
              tab[i].name.indexOf(_self.userInputkey) >= 0 ||
              tab[i].loginName.indexOf(_self.userInputkey) >= 0
            ) {
              _self.userList.push(tab[i]);
            }
          }
        }
      } else {
        _self.userList = _self.userListFull;
      }
    }
  }
};
